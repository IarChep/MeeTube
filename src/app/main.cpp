...
Q_DECL_EXPORT int main(int argc, char *argv[])
{
    ...
    // One-time network environment dump (shared sink "net" category — MEETUBE_DEBUG=net,
    // or =1/all, or the legacy MEETUBE_NET_DEBUG=1). Prints the libcurl
    ...
    if (yt::net::netDebugEnabled()) {
        ...
        qDebug("[net] libcurl %s ssl=%s ipv6=%s http2=%s",
        ...
        qDebug("[net] CA=%s exists=%d readable=%d size=%lld",
                 MEETUBE_CA_BUNDLE, (int) ca.exists(), (int) ca.isReadable(), (qlonglong) ca.size());
#endif
        qDebug("[net] env http_proxy='%s' https_proxy='%s' no_proxy='%s'",
        ...
        qDebug("[net] Qt systemProxy count=%d type0=%d host0=%s "
